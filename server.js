...
import digitalJourneyRouter, {
    assignDigitalPlanForUser,
} from "./routes/digitalJourney.js";
import providerNetworkRouter, {
    assignExpertiseTypesForUser,
} from "./routes/providerNetwork.js";
import userScoresRouter, {
    calculateAndStoreUserScores,
} from "./routes/userScores.js";
...
let corsOptions = {
    origin: [
        "http://localhost:3000",
        "https://metabolicpoint.insolutionsoftware.co.uk",
    ],
};
...
app.post("/submit-health-data", verifyToken, async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: "An error occurred while submitting health data",
        });
    }
});
...
app.get("/average-health-metrics", verifyToken, async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: "An error occurred while fetching average health metrics",
        });
    }
});
...
app.get("/get-user-profile", verifyToken, async (req, res) => {
    try {
        ...
        if (!userID) {
            return res
                .status(400)
                .json({ error: "Missing required parameters: userID" });
        }
    ...
    try {
        ...
        if (!userId || !parameter) {
            return res
                .status(400)
                .json({
                    error: "Missing required parameters: userId or parameter",
                });
        }
...
app.get("/calc-health-score", verifyToken, async (req, res) => {
    try {
        ...
        const calculatedBMI = (
            Number(results[0].weight) /
            (heightInMeters * heightInMeters)
        ).toFixed(1);

        //console.log(process.env.ENABLE_OPENAI_SCORE);

        if (process.env.ENABLE_OPENAI_SCORE == "true") {
            // 0 to skip chatgpt call

            const messages =
                `Give me a json response in the format {score: <50>, description: <description>, lastUpdate: <lastUpdate>, bmi: ${calculatedBMI} } in percentage based on the following parameters: ` +
            ...
            const scoreResult = await calculateAndStoreUserScores(
                userId,
                healthDataForScoring,
            );
        ...
        res.status(500).json({
            error: "An error occurred while calculating health score",
        });
    }
});
...
app.post(
    ...
    async (req, res) => {
        try {
            ...
            if (
                !fileType ||
                !["csv", "xlsx", "xls", "pdf", "jpg", "jpeg", "png"].includes(
                    fileType.ext,
                )
            ) {
                return res
                    .status(400)
                    .json({
                        error: "Invalid file type. Only CSV, Excel, PDF, JPG and PNG files are allowed.",
                    });
            }
                    ...
                    const extractResponse =
                        await openai.chat.completions.create({
                            ...
                            messages: [
                                {
                                    role: "system",
                                    content: `extract the relevant data from the content and the following a json string with the correct values where "Fasting Blood Glucose" equals "Blood Glucose".  If no data is found return 0 for that parameter.  Return a properly formated JSON string that can be parsed in a format like '{
    ...
    "Reference Range": "<25"
                    ...
                    if (extractResponse.code === "insufficient_quota") {
                        return res
                            .status(429)
                            .json({
                                error: "Insufficient quota. Please try again later.",
                            });
                    }
                    ...
                    if (error.status === 429) {
                        return res
                            .status(429)
                            .json({
                                error: "Insufficient quota. Please check your OpenAI account limits.",
                            });
                    }
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [
                    ...
                    results2[0].height,
                    results2[0].weight,
                    results2[0].waistCircumference,
                    results2[0].bloodPressureSystolic,
                    results2[0].bloodPressureDiastolic,
                ],
            );
            ...
            console.log(
                "Assigning digital journey plan for user:",
                UserID,
                "with scores:",
                scoreResult.biomarkerScores,
            );
            try {
                const planId = await assignDigitalPlanForUser(
                    UserID,
                    scoreResult.biomarkerScores,
                );
            ...
            console.log(
                "Assigning expertise types for user:",
                UserID,
                "with scores:",
                scoreResult.biomarkerScores,
            );
            try {
                const expertiseTypes = await assignExpertiseTypesForUser(
                    UserID,
                    scoreResult.biomarkerScores,
                );
...
app.post("/get-data-files", verifyToken, async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: "An error occurred while fetching data files",
        });
    }
});
...
app.post("/auth/login", async (req, res) => {
    try {
        ...
        if (
            ...
            user.last_login_attempt > Date.now() - 15 * 60 * 1000
        ) {
            return res
                .status(403)
        ...
        if (!isPasswordValid) {
            ...
            await pool.execute(
                "INSERT INTO login_history (UserID, success) VALUES (?, ?)",
                [UserID, false],
            );
        ...
        await pool.execute(
            "UPDATE users SET login_attempts = 0, last_login_attempt = CURRENT_TIMESTAMP WHERE UserID = ?",
            [UserID],
        );
        ...
        const token = jwt.sign(
            { id: user.UserID, email: user.UserID },
            process.env.JWT_SECRET,
            {
...
app.post("/auth/refresh", (req, res) => {
    ...
    try {
        // Verify token signature, but ignore expiration
        const payload = jwt.verify(token, process.env.JWT_SECRET, {
            ignoreExpiration: true,
        });

        // Check if token expired too long ago (7 days grace period)
        const now = Math.floor(Date.now() / 1000);
        const timeSinceExpired = now - payload.exp;
        const MAX_GRACE_PERIOD = 7 * 24 * 60 * 60; // 7 days in seconds

        if (timeSinceExpired > MAX_GRACE_PERIOD) {
            return res
                .status(401)
                .json({
...
app.get("/get-reco-actions", verifyToken, async (req, res) => {
    try {
            ...
            recommendations = results
                .map((result) => {
                    ...
                    const numericHealthData = {
                        ...
                        bloodPressureSystolic: parseFloat(
                            healthData.bloodPressureSystolic,
                        ),
                        bloodPressureDiastolic: parseFloat(
                            healthData.bloodPressureDiastolic,
                        ),
                        fastingBloodGlucose: parseFloat(
                    ...
                    if (
                        result.biomarker_name === "weight" &&
                        numericHealthData.weight >= result.range_from &&
                        numericHealthData.weight <= result.range_to
                    ) {
                        return result;
                    } else if (
                        result.biomarker_name === "bloodPressureSystolic" &&
                        numericHealthData.bloodPressureSystolic >=
                            result.range_from &&
                        numericHealthData.bloodPressureSystolic <=
                            result.range_to
                    ) {
                        ...
                        result.biomarker_name === "bloodPressureDiastolic" &&
                        numericHealthData.bloodPressureDiastolic >=
                            result.range_from &&
                        numericHealthData.bloodPressureDiastolic <=
                            result.range_to
                    ) {
                        ...
                        result.biomarker_name === "fastingBloodGlucose" &&
                        numericHealthData.fastingBloodGlucose >=
                            result.range_from &&
                        numericHealthData.fastingBloodGlucose <= result.range_to
                    ) {
                        return result;
                    } else if (
                        result.biomarker_name === "hdlCholesterol" &&
                        numericHealthData.hdlCholesterol >= result.range_from &&
                        numericHealthData.hdlCholesterol <= result.range_to
                    ) {
                        return result;
                    } else if (
                        result.biomarker_name === "triglycerides" &&
                        numericHealthData.triglycerides >= result.range_from &&
                        numericHealthData.triglycerides <= result.range_to
                    ) {
                        return result;
                    } else if (
                        result.biomarker_name === "height" &&
                        numericHealthData.height >= result.range_from &&
                        numericHealthData.height <= result.range_to
                    ) {
                        return result;
                    } else if (
                        result.biomarker_name === "waistCircumference" &&
                        numericHealthData.waistCircumference >=
                            result.range_from &&
                        numericHealthData.waistCircumference <= result.range_to
                    ) {
                        return result;
                    } else if (
                        result.biomarker_name === "vitaminD2" &&
                        numericHealthData.vitaminD2 >= result.range_from &&
                        numericHealthData.vitaminD2 <= result.range_to
                    ) {
                        return result;
                    } else if (
                        result.biomarker_name === "vitaminD3" &&
                        numericHealthData.vitaminD3 >= result.range_from &&
                        numericHealthData.vitaminD3 <= result.range_to
                    ) {
                        return result;
                    }
                    return null;
        ...
        const mytemp = recommendations.map((result) => ({
            title:
                userId !== "all"
                    ? `Your ${result.biomarker_name === "waistCircumference" ? "Waist Height Ratio" : result.biomarker_name} is between ${result.range_from} - ${result.range_to}`
